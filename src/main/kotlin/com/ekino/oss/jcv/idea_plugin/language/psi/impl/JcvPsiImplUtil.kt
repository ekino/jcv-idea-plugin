...
import com.ekino.oss.jcv.idea_plugin.language.psi.JcvElementFactory
...
object JcvPsiImplUtil {
  ...
  fun getParameterValue(element: JcvParameterEntry) =
    element.findDescendantOfType<PsiElement> { it.elementType == JcvTypes.PARAMETER }
  ...
  @JvmStatic
  fun getIndex(element: JcvParameterEntry): Int? = element.parentOfType<JcvParameters>()
  ...
  fun getSeparator(element: JcvParameterEntry) =
    element.findDescendantOfType<PsiElement> {
      it.elementType == JcvTypes.PARAMETER_SEPARATOR
    }

  private fun PsiElement.findValidatorIdElement() =
    findDescendantOfType<PsiElement> { it.elementType == JcvTypes.VALIDATOR_ID }
