...
import com.ekino.oss.jcv.idea_plugin.definition.existsInModule
...
fun findReplacementSuggestions(
  module: Module?,
  jsonValueElements: List<JsonValue>
) = jsonValueElements
...
private fun suggestReplacements(element: PsiElement): List<JcvValidatorSuggestion> =
  when (val value = element as? JsonValue) {
    is JsonStringLiteral -> {
      ...
      val suggestionCategories = mutableListOf<JcvValidatorSuggestion>()

      if (UUID_REGEX.matches(textValue)) {
        suggestionCategories += suggestion(SuggestionCategory.ID, "uuid")
      }

      if (MONGO_ID_REGEX.matches(textValue)) {
        suggestionCategories += suggestion(SuggestionCategory.ID, "mongo_id")
      }
      ...
      DATE_FORMATTERS
        ...
        .also { suggestionCategories += it }

      if (textValue.contains("/")) {
        val category = SuggestionCategory.URL
        if (textValue.isURI()) {
          suggestionCategories += listOf(
      ...
      suggestionCategories += SuggestionCategory.TEXT.let { category ->
        listOf(
